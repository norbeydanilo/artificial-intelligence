...
```bash
python -m ipykernel install --user --name=venv --display-name "nb_ai (venv)"
```
...
Esto abre el navegador con el explorador de Jupyter. Desde ahí seleccionas **New → nb_ai (venv)**.
...
4. En la esquina superior derecha, selecciona el kernel **nb_ai (venv)**.
...
Desde VSCode creas o abres tu `.ipynb` y seleccionas el kernel **nb_ai (venv)**.
